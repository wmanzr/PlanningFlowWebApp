...
export const ExecutorMatchingCard = ({ fullName, username, matchedSkillIds, distanceMeters, workedTodayMinutes, maxDailyLoadMinutes, workloadWeekdayLabel = 'За день задачи', algorithmMiss = false, profileTo, rankBadge, actions, selectable = false, selected = false, onToggleSelect, selectionAddDisabled = false, blockedSelectionTooltip, }: ExecutorMatchingCardProps) => {
    ...
    const hasMetrics = !algorithmMiss && workedTodayMinutes !== undefined && maxDailyLoadMinutes !== undefined;
    ...
    const leftBlock = (<div className="min-w-0">
      ...
      {skillIdsToShow.length > 0 ? (<div className="mt-1 flex flex-wrap gap-1">
          {skillIdsToShow.map((id) => (<Badge key={id} tone="neutral">
      ...
      {algorithmMiss ? (<Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.7rem' }}>
          ...
          {hasMetrics ? (<>
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.7rem' }}>
                {workloadWeekdayLabel}: {formatWorkedHours(workedTodayMinutes)} из{' '}
