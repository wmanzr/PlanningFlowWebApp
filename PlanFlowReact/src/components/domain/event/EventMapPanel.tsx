...
import { Button, Card, MapView, Modal, MAP_ZOOM_DETAIL, MAP_ZOOM_OVERVIEW, resolveMapViewportCenter, type MapMarker, } from '@/components/ui';
    ...
    return (<>
            <Card padded={false} className="flex w-full min-w-0 flex-col overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-secondary/60 px-5 py-4">
                    ...
                    <Button size="sm" variant="secondary" type="button" onClick={() => setMapExpanded((open) => !open)}>
                        {mapExpanded ? 'Свернуть' : 'Развернуть'}
                    </Button>
                </div>
                <div className="px-3 py-3 sm:px-5 sm:py-4">
                    <MapView height="200px" center={mapViewportCenter} zoom={MAP_ZOOM_OVERVIEW} markers={markers} showLegend={markers.length > 1}/>
                </div>
            </Card>

            <Modal open={mapExpanded} onClose={() => setMapExpanded(false)} title={expandModalTitle} description={markerSummary} size="lg">
                {mapExpanded ? (<MapView height="min(480px, 55vh)" center={mapViewportCenter} zoom={MAP_ZOOM_DETAIL} markers={markers} showLegend={markers.length > 1}/>) : null}
            </Modal>
        </>);
}
