...
export function TaskAssignMatchingModal({ open, onClose, taskId, eventId, initialPickCount, onAssigned, }: TaskAssignMatchingModalProps) {
    ...
    useAutoSelectRankedExecutors(matchResult, requiredSlots, setSelectedExecutorIds, open && phase === 'matching');
    useEffect(() => {
            ...
            : 'flex flex-col gap-4'}>
                {phase === 'count' ? (<>
                        ...
                        <div className="flex flex-wrap items-start gap-x-6 gap-y-3">
                            ...
                            <FormControlLabel className="m-0 shrink-0" control={<Checkbox checked={preferCloserCritical} onChange={(_, checked) => setPreferCloserCritical(checked)} size="small"/>} label={<span className="text-sm text-headline">
                                        Искать кто ближе
